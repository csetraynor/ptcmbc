...
functions {
  ...
  real ptcm_log(vector yobs, vector v, vector beta_clin, matrix Z_clin, real alpha, real mu, vector frailty){
      real lpdf;
      real pdf;
      real cdf;
      real term1;
     ...
     for(i in 1:num_elements(yobs)){
        if( exp(-(mu + frailty[i]) ) / alpha) <= 0 || exp(-(mu + frailty[i]) ) / alpha) > 10e10 || alpha <= 0 || alpha > 10e10){
                    prob[i] = 1e-10;  //catch errors
        }else{
          lpdf = weibull_lpdf(yobs[i] | alpha, exp(-(mu + frailty[i]) / alpha) );
          pdf = exp(lpdf);
          term1 = - (log(f[i]))*pdf;
          cdf = weibull_cdf(yobs[i], alpha, exp(-(mu + frailty[i]) / alpha) );
...
data {
  ...
  int<lower=1, upper=4> ctype[N]; //cancer type
}
...
model {
  ...
  yobs ~ ptcm(v, beta_clin, Z_clin, alpha, mu, frailty);
