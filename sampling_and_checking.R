...
## ----sim-data-function-----------------------------------------------
fun_sim_data <- function(alpha, N, beta, gamma , icept, X, a) {
  
  b0 <- icept
  risk <- X %*% beta #prognostic index
  f <- exp(b0 + risk) #link function mixed model
  th <- X %*% gamma #link function time promotion 
  ...
  #inverse function
  term1 <- log(1 - ( (1 - (spop)^(a) ) / ( a * f[cure == F] / (1 + a * f[cure == F]))) )
  sigma <- exp( -(th[cure == F])/alpha)
  dfs <- ( - term1 * sigma ) ^ (1 / alpha)
